...
namespace emmVRC.Functions.Core
{
    public class Hooking : MelonLoaderEvents
    {
        ...
        public override void OnApplicationStart()
        {
            ...
            try
            {
                Main.instance.HarmonyInstance.Patch(typeof(VRC_StationInternal).GetMethod("Method_Public_Boolean_Player_Boolean_0"), new HarmonyLib.HarmonyMethod(typeof(Hooking).GetMethod("PlayerCanUseStation", System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Static)));
            ...
            try
            {
                foreach (MethodInfo method in typeof(PlayerNameplate).GetMethods(BindingFlags.Public | BindingFlags.Instance).Where(x => methodMatchRegex.IsMatch(x.Name)))
                {
                    ...
                    Main.instance.HarmonyInstance.Patch(method, null, new HarmonyLib.HarmonyMethod(typeof(Hooking).GetMethod("OnRebuild", BindingFlags.NonPublic | BindingFlags.Static)));
                }
            }
